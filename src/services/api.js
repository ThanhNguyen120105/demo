...
// Medical Result API - Lấy kết quả xét nghiệm
export const medicalResultAPI = {
  // Lấy chi tiết kết quả xét nghiệm theo medicalResultId
  getMedicalResultById: async (medicalResultId) => {
    try {
      console.log('Getting medical result by ID:', medicalResultId);
      ...
      console.error('Get medical result error:', error);
      
      let errorMessage = 'Không thể lấy kết quả xét nghiệm';
      
      if (error.response?.status === 404) {
        errorMessage = 'Không tìm thấy kết quả xét nghiệm';
      } else if (error.response?.status === 403) {
        errorMessage = 'Bạn không có quyền xem kết quả xét nghiệm này';
      } else if (error.response?.status === 401) {
        errorMessage = 'Bạn cần đăng nhập để xem kết quả xét nghiệm';
      } else if (error.response?.data?.message) {
        errorMessage = error.response.data.message;
      }
